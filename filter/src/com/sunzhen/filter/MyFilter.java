 ...
 *              <filter-class>com.sunzhen.filter.MyFilter</filter-class>  配置filter指定的类名
 *          </filter>
 *          <filter-mapping>
 *              <filter-name>filter</filter-name>       配置filter的别名，跟filter标签中的name对应
 *              <url-pattern>/*</url-pattern>           表示要匹配的请求 或者*.do
 *          </filter-mapping>
 *      生命周期:
 *          从tomcat启动到tomcat关闭
 *
...
public class MyFilter implements Filter {
    ...
    public void init(FilterConfig filterConfig) throws ServletException {
        System.out.println("我是 filter init");
    }

    @Override
    public void doFilter(ServletRequest servletRequest, ServletResponse servletResponse, FilterChain filterChain) throws IOException, ServletException {
        System.out.println("我是 filter逻辑处理");
        //添加此语句之后才能调用到对应的servlet
        filterChain.doFilter(servletRequest,servletResponse);
        System.out.println("我是 filter逻辑处理结束");
    }

    @Override
    public void destroy() {
        System.out.println("我是 filter销毁功能");
    }
}
